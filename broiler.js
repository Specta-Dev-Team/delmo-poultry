(function () {
  const CONFIG = {
    ...
    cardActions: ["batchCreation", "mortality", "feedMedication", "bodyWeight", "water", "lighting", "environment"],
    categoryFilters: [
      { key: "all", label: "All" },
  ...
  const houses = [
    {
        ...
        users: 3,
        houseCost: 45000,
        batchId: "BR BATCH 003",
        flockAge: "12 days",
        feedIndent: "IND000015",
        birdsHoused: 12500,
        femaleBirds: 6200,
        maleBirds: 6300,
        liveHens: 6200,
        totalEggsProduced: 0,
        totalFeedGivenKg: 725,
        deadBirds: 3,
        totalBirds: 12500,
        sampledBirds: 400,
        birdsWithinTargetWeight: 356,
        mortalityToday: 3,
        trays: {
            standard: 0,
            reject: 0
        ...
        schedules: [
            {
                ...
                dose: "Via water",
                method: "Water line",
                description: "Weekly vitamin boost for immune support",
                status: "Due today"
            }
        ],
        unit: "Unit 1"
    },
    {
        ...
        users: 3,
        houseCost: 52000,
        batchId: "BR BATCH 004",
        flockAge: "24 days",
        feedIndent: "IND000016",
        birdsHoused: 13200,
        femaleBirds: 6500,
        maleBirds: 6700,
        liveHens: 6500,
        totalEggsProduced: 0,
        totalFeedGivenKg: 1188,
        deadBirds: 1,
        totalBirds: 13200,
        sampledBirds: 420,
        birdsWithinTargetWeight: 381,
        mortalityToday: 1,
        trays: {
            standard: 0,
            reject: 0
    ...
    },
    {
        id: "HUS003",
        code: "HUS003",
        name: "Broiler House C",
        stage: "Finisher",
        status: "Active",
        users: 4,
        houseCost: 48000,
        batchId: "BR BATCH 005",
        flockAge: "38 days",
        feedIndent: "IND000017",
        birdsHoused: 11800,
        femaleBirds: 5850,
        maleBirds: 5950,
        liveHens: 5850,
        totalEggsProduced: 0,
        totalFeedGivenKg: 1652,
        deadBirds: 4,
        totalBirds: 11800,
        sampledBirds: 380,
        birdsWithinTargetWeight: 335,
        mortalityToday: 4,
        trays: {
            standard: 0,
            reject: 0
        ...
        schedules: [
            {
                ...
                age: 38,
                date: "05/11/2026",
                name: "Pre-slaughter check",
                dose: "Batch",
                method: "Visual",
                description: "Final health inspection before slaughter",
                status: "Due today"
            }
        ],
        unit: "Unit 1"
  ...
  async function loadUnitOptionsFromDataSource() {
    try {
      if (!window.BroilerAPI || typeof window.BroilerAPI.loadUnitOptions !== "function") return;
  ...
  async function loadHouseDetailData(house) {
    ...
    try {
      if (typeof window.BroilerAPI.loadSchedules === "function") {
        const schedules = await window.BroilerAPI.loadSchedules({ house_id: house.id, batch_id: house.batchId });
        if (schedules.length) {
  ...
  function updateUnitFilters() {
    ...
    const units = getActiveUnitOptions();
...
(function () {
  const AXPERT_DATASOURCES = {
    ...
    houses: { name: "poultry_broiler_houses" },
    charts: { name: "poultry_broiler_charts" },
    schedules: { name: "poultry_broiler_schedules" },
  };
  ...
  async function loadRows(dataSourceName, parameters) {
    ...
    if (!rows.length) {
      try {
        ...
        console.warn(`[Broiler API] Unable to call Axpert data source '${dataSourceName}'.`, error);
      }
    }
