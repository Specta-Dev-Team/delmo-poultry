...
assert.match(source, /readyToHarvestUpdate:\s*\{\s*name:\s*"poultry_cull_harvest_update"\s*\}/, "Grand Parent should configure the ready-to-harvest update datasource");
assert.match(source, /liveBirdTransferRequest:\s*\{\s*title:\s*"Live Bird Transfer Request",\s*transid:\s*"nlbdt"\s*\}/, "Grand Parent should configure the Live Bird Transfer Request tstruct");
assert.match(mapBatchRow, /readyToHarvest:\s*isCompletedFlag\(rowText\(row,\s*\["readytoharvest", "readyToHarvest", "harvestready"\]\)\)/, "Grand Parent batch mapping should preserve the harvestready flag");
assert.match(renderHouseCard, /renderTransferAction\(house\)/, "Grand Parent cards should render the transfer-stage action");
assert.match(renderHouseCard, /ready-harvest-card|live-transfer-card/, "Grand Parent cards should expose a workflow stage class");
...
assert.match(source, /renderReadyToTransferConfirmation|readyToTransferConfirmation/, "Grand Parent should render a ready-to-transfer confirmation");
...
assert.match(completeReadyToTransfer, /await window\.GrandparentAPI\.executeReadyToTransferUpdate\(\{\s*batchid:\s*batchId\s*\}\)/, "Grand Parent should execute the ready-to-transfer update");
assert.match(completeReadyToTransfer, /house\.readyToHarvest\s*=\s*true/, "Grand Parent should switch the card to Live Bird Transfer Request after success");
assert.match(styles, /\.ready-harvest-card/, "Grand Parent should include the blue pending stage styling");
assert.match(styles, /\.live-transfer-card/, "Grand Parent should include the green live-transfer stage styling");
...
assert.match(styles, /\.ready-to-transfer-confirmation/, "Grand Parent should style the confirmation dialog");

console.log("grandparent ready-to-transfer action test passed");
