(function () {
  ...
  function renderListNavbar() {
    moduleCommand.innerHTML = `
      <div class="module-copy">
        ...
        <p><span id="activeHouseCount">${formatNumber(parentHouses.filter((house) => house.status === "Active").length)}</span> active houses / <span id="birdTotal">${formatNumber(parentHouses.reduce((sum, house) => sum + house.birdsHoused, 0))}</span> birds housed</p>
      </div>
      ${renderSearchControl()}
      ${renderListActions()}
      ${renderNavTools()}
  ...
  function openTstruct(actionKey, context) {
    ...
    tstructFrame.src = buildTstructUrl(action.transid, context || { module: "parent" });
  ...
  function renderHouseDetail(houseId) {
    const house = parentHouses.find((item) => item.id === houseId);
    if (!house) return;
  ...
  function updateUnitFilters() {
    const unitCounts = parentHouses.reduce((acc, house) => {
      acc.all = (acc.all || 0) + 1;
  ...
  function renderHouses(query) {
    ...
    const visibleHouses = parentHouses.filter((house) => {
      const matchesUnit = selectedUnit === "all" || house.unit === selectedUnit;
      const matchesSearch = !search || [house.name, house.code, house.stage, house.batchId, house.status, house.unit]
        .join(" ")
        .toLowerCase()
        .includes(search);
    ...
    document.getElementById("activeHouseCount").textContent = formatNumber(parentHouses.filter((house) => house.status === "Active").length);
    document.getElementById("birdTotal").textContent = formatNumber(parentHouses.reduce((sum, house) => sum + house.birdsHoused, 0));
    updateUnitFilters();
